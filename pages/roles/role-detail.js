...
async function load() {
  ...
  byId('detail-title').textContent = role.name;
  ...
  wireRelation({
    ...
    label: permLabel, readOnly: role.isSystemRole, reload: load,
    add: (permId) => roles.grantRolePermission(id, permId), remove: (permId) => roles.revokeRolePermission(id, permId),
  });
}
