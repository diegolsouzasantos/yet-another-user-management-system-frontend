...
function renderBack() {
  ...
  const link = createEl('a', { className: 'detail-back', href: '/pages/users/users.html' });
...
function renderPermissions(user, catalog, reload) {
  const note = byId('perms-note');
  note.hidden = !user.roleGrantsAllPermissions;
  if (user.roleGrantsAllPermissions) note.textContent = t('users.roleGrantsAll');

  const canRevoke = can('permissions:revoke');
  const rows = user.permissions.map((permission) => {
    const cells = [
      createEl('td', { textContent: permLabel(permission) }),
...
async function init() {
  ...
  can = (permission) => actor.grantsAll || actor.permissions.includes(permission);
